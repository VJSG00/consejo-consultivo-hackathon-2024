...
export const createGestor = async (req: Request, res: Response) => {
    try {
        ...
        await Usuarios.create({ email, password: '', role });
...
export const login = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const token = jwt.sign({ id: data.id, role: data.role, email: data.email }, process.env.JWT_SECRET, { expiresIn: '24h' });
        
        res.status(200).json({ message: 'Inicio de sesión exitoso', token });
    } catch (error) {
        res.status(500).json({ error: "La cuenta no está confirmada o el usuario no existe" });
    }
};
